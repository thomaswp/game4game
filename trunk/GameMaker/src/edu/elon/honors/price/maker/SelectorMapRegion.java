...
public class SelectorMapRegion extends SelectorMapBase {
	...
	protected static class RegionView extends MapView {
		...
		public void drawGrid(Canvas c) {
			...
			Rect selection = getNormSelection();
			selection.offset((int)offX, (int)offY);
			
			paint.setColor(selectionFillColor);
			paint.setStyle(Style.FILL);
			c.drawRect(selection, paint);
...
//			c.drawText(text, 0, paint.getTextSize(), paint);
			...
			String text = topLeftString(selection);
			c.drawText(text, selection.left + offX - paint.measureText(text) / 2, 
					selection.top + offY - 5, paint);
			
			text = bottomRightString(selection);
			c.drawText(text, selection.right + offX - paint.measureText(text) / 2, 
					selection.bottom + offY + paint.getTextSize() + 5, paint);
		}
